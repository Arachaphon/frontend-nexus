# 🏢 Dormitory Management System

โปรเจกต์รายวิชา **Back-End Software Development** และ **Software Development Operation** พัฒนาโดยกลุ่ม **The Nexus** สาขาวิศวกรรมซอฟต์แวร์ มหาวิทยาลัยพะเยา | ปีการศึกษา 2568

เว็บไซต์บริหารจัดการหอพักที่ช่วยให้การจัดการข้อมูลผู้เช่า ห้องพัก และการคำนวณค่าใช้จ่ายรายเดือนเป็นเรื่องง่ายและมีประสิทธิภาพ

---

## 💡 ที่มา (Inspiration)
การบริหารจัดการหอพักในรูปแบบเดิมมักประสบปัญหาเรื่องความซับซ้อนของข้อมูล ไม่ว่าจะเป็นการบันทึกสถานะห้องพัก การคำนวณค่าน้ำ-ค่าไฟที่คลาดเคลื่อน 

กลุ่ม **The Nexus** จึงได้พัฒนาระบบนี้ขึ้นเพื่อเปลี่ยนการทำงานให้เป็นระบบดิจิทัล (Digital Transformation) เพื่อช่วยลดภาระงาน เพิ่มความแม่นยำของข้อมูล 

---

## 🎯 วัตถุประสงค์ (Objectives)
- **Centralized Data:** เพื่อจัดเก็บข้อมูลห้องพักและผู้เช่าอย่างเป็นระบบในฐานข้อมูลเดียว
- **Automation:** เพื่อคำนวณค่าใช้จ่ายรายเดือนและออกใบแจ้งหนี้ (Invoice) โดยอัตโนมัติ
- **Better Communication:** เพิ่มช่องทางการแจ้งซ่อมและการประกาศข่าวสารที่เข้าถึงง่าย
- **Real-time Monitoring:** ช่วยให้ผู้ดูแลเห็นภาพรวมรายได้และสถานะห้องว่างได้ทันที

---

## 🌐 Demo
🔗 [ทดลองใช้งานระบบจัดการหอพัก](https://thenexus-chi.vercel.app/)

---
## 🛠 Forntend github and Forntend gitLab
🔗 https://github.com/Arachaphon/backend-nexus.git
🔗 https://gitlab.com/thenexus1/frontend-nexus.git
## 🛠 Backend github and Backend gitLab
🔗 https://github.com/Arachaphon/backend-nexus.git
🔗 https://gitlab.com/thenexus1/backend-nexus.git

---

## 🛠 Tech Stack (Tools)
- **React** – Framework สำหรับพัฒนา User Interface แบบ Reactive
- **Tailwind CSS** – สำหรับการออกแบบดีไซน์ที่รองรับทุกหน้าจอ (Responsive Design)
- **TypeScript** – เพื่อเพิ่มประสิทธิภาพในการจัดการโครงสร้างข้อมูลและลดข้อผิดพลาด
- **CloundFlare D1** – Serverless SQLite database

---

## ✨ ฟีเจอร์หลัก (Key Features)

| Frontend | Backend | Feature Name | Description |
|----------|---------|--------------|-------------|
| ✅ | ✅ | **Create Dormitory** | สร้างหอพัก |
| ✅ | ✅ | **Room Management** | จัดการสถานะห้องพัก (ว่าง/จอง/เต็ม) และข้อมูลราคา |
| ✅ | ✅ | **Billing System** | บันทึกมิเตอร์น้ำ-ไฟ และคำนวณค่าใช้จ่ายอัตโนมัติ |
| ✅ | ✅ | **Maintenance** | ระบบแจ้งซ่อมสำหรับผู้เช่าและระบบจัดการสถานะสำหรับเจ้าของ |
| ✅ | ✅ | **Contract & Tenants** | ระบบจัดการสัญญาเช่าและฐานข้อมูลผู้พักอาศัย |


---

## 📝 บทบาทผู้ใช้งาน (User Roles)

| บทบาท | คำอธิบาย |
|---------|------------|
| **User** | มีสิทธิ์สร้างหอพักและจัดการโปรไฟล์ส่วนตัว |
| **Owner** | มีสิทธิ์การจัดการหอพักและจัดการบัญชีผู้ใช้งานภายในหอพักได้ |
| **Manager** | มีสิทธิ์การจัดการหอพักทั้งหมดที่ได้รับอนุญาต |
| **Staff** | มีสิทธิ์จัดการจัดการหอพักทั้งหมดที่ได้รับอนุญาตยกเว้นข้อมูลการตั้งค่าหอพัก |

---

## 👨‍💻 ทีมผู้พัฒนา (The Team)

| รหัสนิสิต | ชื่อ-นามสกุล | บทบาทหน้าที่ |
|-------------|----------------------|----------------------|
| 67022535 | Karunyaphat Kanthanate | UX/UI Design & Frontend Developer |
| 67022748 | Peerapat Sawaengram | Project Coordinator & Frontend Developer |
| 67023031 | Arachaphon Klinchuen | Backend Developer & Frontend Developer |


---

## 📞 ติดต่อสอบถาม
หากมีคำถามหรือข้อเสนอแนะเกี่ยวกับโปรเจกต์ สามารถติดต่อได้ที่:
- **E-mail:** 67022748@up.ac.th
- **Phone:** 082-181-9636
